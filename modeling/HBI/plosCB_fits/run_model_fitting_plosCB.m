...
paramcolors = parula(12);

% WHICH PARAMETER COMBINATIONS DO YOU WANT?
% Models are specified by which parameters you want to include, like so:
% "epsilon_initi_alpha_uc"
% This is a model containing a noise term epsilon (called "sigma" in the paper)
% initial rating for each task initi, a learning rate alpha which dictates
% the speed of cost learning, and a update cost only (most frequently
...
paramsofinterest = {'uc','mainc','lurec','missc','fac','respc',...
    'alpha','initi'};
...
modelstofit = [modelstofit get_all_param_combos(paramsofinterest)]; 
...
if strict_criterion_for_fitting
    ...
    recoverability = false(length(modelstofit),1);
...
new_cost = '';
if fitflag
    for m = 1:3 %length(modelstofit)
        % don't fit redundantly, to save time. just fit new models which
        % have not been fit to real subject data yet.
        if contains(modelstofit{m},new_cost)
            disp(['Model #' num2str(m)])
            disp(modelstofit{m})
            cbm_lap(data,funcs{m},priors{m},fnames{m});
            
            % COPY LOW LEVEL FITS OVER TO LOW-LEVEL-FITS DIRECTORY!
            % the easy log likelihood values get overwritten by other outputs
            % in the hierarchical version of model fits
            copyfile([modelstofit{m} '.mat'],['../low-level-fits/' modelstofit{m} '.mat'])
        end
...
% if you want to run new hierarchical model fits, then name a new fname_hbi
% to separate it from the others.
% if you want to view old, pre-saved fits, then use one of the
% following names:

% 2023 bioRxiv version
fname_hbi = 'different_deltas_versus_winning_models_strict.mat';

% 2024 plos cb supplement version
% fname_hbi = 'PloS_CB_revisions_2024.mat';
...
if ~HPC_flag
    ...
    best_model.cbm = cbm;
    %save('HBI_modelStruct_2024.mat','best_model');
    ...
    % calculate how much more variance is explained by the inclusion of
    % each parameter
    % additional_r_squared_by_parameter()
    
    % Plot individual model information if you want confirmation it's fitting
    % well
    m = best;
    addpath('../model-details/');

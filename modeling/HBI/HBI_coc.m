...
%add relevant paths
addpath('cbm-master/codes/');addpath('model_functions/');
addpath('../'); %other COC code, like model loading
load('../../simdata/toanalyze.mat'); 
% grab task characteristics from real subjects
model_detail_folder = dir('model_details'); list_existing = cellstr(string(char(model_detail_folder.name)));
function_folder = dir('model_functions'); list_existing_functions = cellstr(string(char(function_folder.name)));
...
for m = 1:length(modelstosim)
    ...
    if sum(contains(list_existing,[model_name '.mat' repmat(' ',1,diff)]))==0 || forcefit %have you run gen-rec on this model already?
        ...
        save(['model_details/' modelstofit{m}],'fitparams','realparamlist','subset','data','subsetdata')
    else
        load(['model_details/' modelstofit{m}])
    end
    figure(1)
    ...
    %reliable = 'n'; close 1
    save(['model_details/' modelstosim{m}],'realparamlist','fitparams','subset','reliable','data','subsetdata')

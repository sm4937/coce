...
modelstofit = [get_all_param_combos({'uc','initi'}) get_all_param_combos({'lurec','initi'}) get_all_param_combos({'fac','initi'}) modelstofit];
...
for m = 12:24  %length(modelstosim)
...
for m = 12:24
    ...
    for p = 1:nparams
        subplot(5,3,p)
        scatter(realparamlist(:,p),fitparams(:,p),[],rand(length(subset),3),'Filled')
        hold on
        plot([0 1],[0 1],'--')

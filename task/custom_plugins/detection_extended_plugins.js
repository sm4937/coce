...
function setup_detection_n(practice_flag,loopi,n){ //called n-back but it's both that and the combine
  ...
  var instructs = {
    timeline: [{
      ...
      stimulus: function() {
        ...
        var instructs_text = '<p>In this task, you will monitor the screen for target sequences of letters. You need to press <strong>' + answer_key_names[0] + '</strong> every time you see the same letter ' + String(n+1) + ' times in a row.</p><p>If the letter on screen is not the same one you saw ' + String(n-1) + ' and ' + String(n) + ' letters ago, do not press anything.</p><p>There may be be multiple targets in a row.</p><p>Press any key to begin.</p>'; 
  ...
  for(var trial = 0; trial < ntrials; trial++){
    ...
    //make it flexible
    stimolds = [stimnum_list[trial-n], stimnum_list[(trial-n)+1]]; //this is fixed for now but it shouldn't be
    if(trial_list[trial]==1){ //force it to be an n-back trial
      stimnum_list[trial] = stimolds[0];
    ...
    if(stimnum==stimolds[0]&stimnum==stimolds[1]){
  ...
  timeline = [instructs,detection,debrief];
  return timeline;
...
function accuracyNdetect(data){ //calculate accuracy for Nback
  ...
  lasttrialdata = jsPsych.data.getLastTimelineData().filter({task: 'detection_n'});
